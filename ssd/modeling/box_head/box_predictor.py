...
class BoxPredictor(nn.Module):
    ...
    def forward(self, features,mask_p):
        cls_logits = []
        bbox_pred = []
        for feature, cls_header, reg_header in zip(features, self.cls_headers, self.reg_headers):
            cc =cls_header(feature)
            self.interpolate = nn.AdaptiveAvgPool2d((feature.shape[2], feature.shape[3]))
            Mask = self.interpolate(mask_p)

            #mask[81]
            M = Mask.repeat(1,int(cc.shape[1]/81),1,1)

            cls_logits.append((cc*M).permute(0, 2, 3, 1).contiguous())
